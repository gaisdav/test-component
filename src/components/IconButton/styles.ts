...
import { Button as MuiButton, IButtonProps } from '../Button';
...
export const StyledIconButton = styled(MuiButton)<IButtonProps>`
  min-width: 36px;
  padding: 0;
  color: ${(props) => props.theme.custom.lightTheme.colors.typographyAndIcons.default.active};
  ...
  ${({ size = 'small' }) => css` ${IconButtonSizeVariants[size]}; `};
  ${({ assign = 'outlined' }) => css` ${IconButtonColorVariants[assign]}; `};
`;
